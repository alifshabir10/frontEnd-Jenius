...
export const AllCont = props => {
    let history = useHistory;
	

	const onClickAddCont = () => {
		history.push('/addContact')
	  }
    const [data, setData] = useState([]);
    useEffect(() => {
        setData(props.cont.data)
        ...
        <div className="py-8">
            ...
            <div className="my-2 flex sm:flex-row flex-col">
                ...
                <div className="flex justify-end ">
                    <span className="h-full absolute inset-y-0 left-0  items-center pl-2">
                        <svg viewBox="0 0 24 24" className="h-4 w-4 fill-current text-gray-500">
                            <path
                                d="M10 4a6 6 0 100 12 6 6 0 000-12zm-8 6a8 8 0 1114.32 4.906l5.387 5.387a1 1 0 01-1.414 1.414l-5.387-5.387A8 8 0 012 10z">
                            </path>
                                    ...
                                    data.map((cont, index) => {
                                        return (
                                            <tr key={cont.id}>
                                                ...
                                                <td className="px-5 py-5 border-b border-gray-200 bg-white text-sm">
                                                    <p className="text-gray-900 whitespace-no-wrap">{cont.firstName}</p>
                                                </td>
                                                <td className="px-5 py-5 border-b border-gray-200 bg-white text-sm">
                                                    <p className="text-gray-900 whitespace-no-wrap">
                                                        {cont.lastName}
                                                ...
                                                <td className="px-5 py-5 border-b border-gray-200 bg-white text-sm">
                                                    <span
                                                        ...
                                                        <span className="relative">{cont.age}</span>
                                                    </span>
                                                </td>
                                                <td className="px-5 py-5 border-b border-gray-200 bg-white text-sm">
                                                ...
                                                <td className="px-5 py-5 border-b border-gray-200 bg-white text-sm">
                                                    ...
                                                    <button onClick={onClickAddCont}
                                                        className="text-gray-600 bg-transparent border border-solid border-gray-300 hover:bg-gray-600 hover:text-white active:bg-blue-600 font-bold uppercase text-xs px-4 py-2 rounded outline-none focus:outline-none mr-1 mb-1 ease-linear transition-all duration-150" type="button">
                                                        Edit
                                    </button>
                                                    <button onClick={() => {
                                                        if (window.confirm("apakah anda yakin ingin menghapus data ini?")) {
                                                            // props.setDelete(cont.id);
                                                        }
